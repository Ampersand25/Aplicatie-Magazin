...
void TestingDomain::runTestsDomainDefaultConstructor() const noexcept
{
	Product p;

	assert(!p.getName().size());     // assert(p.getName() == "");
	assert(!p.getType().size());     // assert(p.getType() == "");
	assert(p.getPrice() >= 1e-12);
	assert(!p.getProducer().size()); // assert(p.getProducer() == "");
}
...
{
	runTestsDomainDefaultConstructor();
